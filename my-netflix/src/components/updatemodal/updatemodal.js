...
function UpdateModal( {show,handleClose,ele,getFavMovie,
    ...
    const update = async(id)=>{
        ...
        let updated = {title:titleInput,overview:ele.overview, release_date:ele.release_date , poster_path:ele.poster_path,image:imageInput, comment:commentInput}
        
      await axios.put(`https://bashar-abb.herokuapp.com/updatemovie/${id}`,updated)
                  .then(()=>{
                    getFavMovie();
                  }).catch(err=>{
                      console.log(err);
